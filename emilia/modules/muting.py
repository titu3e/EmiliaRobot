...
def check_user(user_id: int, bot: Bot, chat: Chat) -> Optional[str]:
    ...
    if not user_id:
        reply = "Pengguna tidak ditemukan"
        return reply
    ...
    if user_id == bot.id:
        reply = "Aku tidak akan MUTE diri sendiri! 😠"
        return reply
...
def mute(update: Update, context: CallbackContext) -> str:
    ...
    if member.can_send_messages is None or member.can_send_messages:
        ...
        msg = (
            f"{mention_html(member.user.id, member.user.first_name)} Sekarang Dibisukan 🤫"
            )
        if reason:
        ...
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "Membunyikan", callback_data="unmute_({})".format(member.user.id))
        ]])
        bot.sendMessage(
            chat.id,
            msg,
...
def temp_mute(update: Update, context: CallbackContext) -> str:
    ...
    try:
        if member.can_send_messages is None or member.can_send_messages:
            ...
            msg = (
                f"{mention_html(member.user.id, member.user.first_name)} Sekarang Dibisukan 🤫"
                f"\n\nUntuk Waktu: (<code>{time_val}</code>)\n"
            )
        ...
        LOGGER.exception(
            "KESALAHAN mematikan pengguna %s pada obrolan %s (%s) karena %s",
            user_id,
            chat.title,
            chat.id,
            excp.message,
    ...
    if match:
        ...
        if unmuted:
        	update.effective_message.edit_text(
        	    f"{mention_html(member.user.id, member.user.first_name)} Sekarang bisa bicara lagi 🤭",
        	    parse_mode=ParseMode.HTML,
        	)
        	query.answer("Disuarakan!")
        	return (
                    f"<b>{html.escape(chat.title)}:</b>\n" 
                    f"#UNMUTE\n" 
                    f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
        ...
        update.effective_message.edit_text(
            "Pengguna ini tidak dibisukan atau telah meninggalkan grup!"
        )
        return ""
