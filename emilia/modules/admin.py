...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        f"Promoting a user in <b>{chat.title}</b>\n\nUser: {mention_html(user_member.user.first_name)}\nAdmin: {mention_html(user.first_name)}",
        parse_mode=ParseMode.HTML,
    )
    ...
    log_message = (
        ...
        f"<b>User:</b> {mention_html(user_member.user.first_name)}"
    )
...
def fullpromote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        f"Fullpromoting a user in <b>{chat.title}</b>\n\n<b>User: {mention_html(user_member.user.first_name)}</b>\n<b>Promoter: {mention_html(user.first_name)}</b>",
        parse_mode=ParseMode.HTML,
    )
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"Sucessfully demoted a admins in <b>{chat.title}</b>\n\nAdmin: <b>{mention_html(user_member.user.first_name)}</b>\nDemoter: {mention_html(user.first_name)}",
            parse_mode=ParseMode.HTML,
        )
        ...
        log_message = (
            ...
            f"#DEMOTED\n"
            f"<b>Admin:</b> {mention_html(user.first_name)}\n"
            f"<b>User:</b> {mention_html(luser_member.user.first_name)}"
        )
...
def set_title(update: Update, context: CallbackContext):
    ...
    bot.sendMessage(
        chat.id,
        f"Sucessfully set title for <code>{user_member.user.first_name}</code> "
...
def unpin(update: Update, context: CallbackContext):
    ...
    log_message = (
        ...
        f"<b>Admin:</b> {mention_html(html.escape(user.first_name))}"
    )
    ...
    if match:
        ...
        if demoted:
        	update.effective_message.edit_text(
        	    f"Admin {mention_html(user.first_name)} Demoted {mention_html(member.user.first_name)}!",
        	    parse_mode=ParseMode.HTML,
        	)
        	...
        	return (
                    ...
                    f"#DEMOTE\n" 
                    f"<b>Admin:</b> {mention_html(user.first_name)}\n"
                    f"<b>User:</b> {mention_html(lmember.user.first_name)}"
                )
    else:
        update.effective_message.edit_text(
